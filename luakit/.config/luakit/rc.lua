...
modes.add_binds("normal", {
    { "ZR", "Restart browser (reload config files).", function(w, o)
        w:restart(o.bang)
    end },
    { 'O', 'Open one or more URLs in a new tab.', function(w)
        w:enter_cmd(":tabopen ")
    end },
    { "wi", "Toggle the DOM inspector.", function(w)
        local v = w.view
        if v.inspector then
            v.close_inspector()
        else
...
webview.add_signal("init", function(view)
    view:add_signal("navigation-request", function(v, uri)
        ...
        if uri == "about:blank" then
            local html = "<html><body bgcolor='#090c1b'></body></html>"
            view:load_string(html, "about:blank")
            return true
        end
        if string.match(string.lower(uri), "^mailto:") then
            luakit.spawn(string.format("%s %q", "st -e neomutt", uri))
            return false
        end
        if string.match(string.lower(uri), "^magnet:") then
            luakit.spawn(string.format("%s '%q'", "qbittorrent-nox", uri))
            w:notify("Downloading torrent with qBittorrent")
            return false
        end
    end)
end)
local video_cmd_fmt = "mpv --ytdl --ytdl-format='bestvideo[height=?250][vcodec!=?vp9]+bestaudio/best' '%s'"
