...
function draw_preview {
    calculate_position
    image_path="$(mktemp /tmp/fzfmovie.XXXXXXX.png)"
    ffmpegthumbnailer -q10 -f -s0 -i "${@}" -o $image_path 2&>/dev/null
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    print_on_winch "${REDRAW_KEY}${REDRAW_KEY}" 2&>/dev/null
    start_ueberzug

    export -f draw_preview calculate_position
    video=$(fd '.(mkv|mp4|avi)$' ~/Series ~/Downloads ~/Movies -t f --color=always | \
        SHELL="${BASH_BINARY}" \
