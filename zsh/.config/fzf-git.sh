...
fzf_gh() {
  ...
  fzf-down --ansi --no-sort --reverse --multi --inline-info \
...
fzf_gr() {
  ...
  fzf-down --tac \
    --preview 'git log --oneline --graph --color=always --date=short --pretty="format:%C(green)%C(bold)%cd %C(auto)%h%d %s (%an)" {1}' |
  cut -d$'\t' -f1
}

fzf_gs() {
  is_in_git_repo || return
  git stash list | fzf-down --reverse -d: --preview 'git show {1} | delta' |
  cut -d: -f1
}
